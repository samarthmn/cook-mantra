...
@pytest.mark.asyncio
async def test_codex_close_failure_still_shuts_down_artifact_storage(
    project_tmp_path: Path,
    ...
    class FailingCodexOwner:
        async def aclose(self) -> None:
            events.append("codex.close")
            raise RuntimeError("codex close failed")

    async def recording_artifact_shutdown() -> None:
        events.append("artifact.shutdown")
        await original_artifact_shutdown()

    app.state.codex_app_server_client = FailingCodexOwner()
    monkeypatch.setattr(artifact_store, "shutdown", recording_artifact_shutdown)

    with pytest.raises(RuntimeError, match="codex close failed"):
        async with app.router.lifespan_context(app):
            pass

    assert events == ["codex.close", "artifact.shutdown"]
    assert app.state.cleanup_task.done()
    assert app.state.job_runner.active_count == 0
